...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str,
                        default=os.environ.get('SM_CHANNEL_TRAIN', '../input/data/merge_dataset'))
...
def do_training(data_dir, model_dir, device, image_size, input_size, num_workers, batch_size,
    ...
    for epoch in range(max_epoch):
        ...
        if valid_avail:
            ...
            model.eval()
            epoch_loss, epoch_start, val_epoch_loss = 0, time.time(), 0
            with tqdm(total=num_batches) as pbar_valid:
                for img, gt_score_map, gt_geo_map, roi_mask in valid_loader:
                    pbar_valid.set_description('[Epoch {}]'.format(epoch + 1))
                    ...
                    loss_val = loss.item()
                    epoch_loss += loss_val
                    val_epoch_loss += extra_info['iou_loss']
                    ...
                    pbar_valid.set_postfix(val_dict_1)
        ...
        if (epoch + 1) % save_interval == 0:
            ...
            cur_iou_loss = (val_epoch_loss/num_batches)
...
def main(args):
    wandb.init(project="ocr",  entity="ptop", config=args, name = 'hyunsoo_E500_dataExceptNoise_')
    np.random.seed(15)
